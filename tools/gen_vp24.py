...
def common_after_vol(session=False):
    ...
    w()
    w('// ---------- 绘制线条 ----------')
    w("// AiCoin 要求脚本必须调用 plot / plotShape 等绘图函数，否则报「无绘图函数」。")
    w("// 先用最简 plot(..., title=...) 保证能跑通；若需要样式再补 color / linewidth 参数。")
    w("plot(VAH_, title='VAH')")
    w("plot(VAL_, title='VAL')")
    w("plot(POC_, title='POC')")
    w()
    w('// ---------- 区域判定 ----------')
    ...
    w()
    w('// ---------- 进出场信号 ----------')
    w('// AiCoin 示例中使用 cross_up / cross_down（带下划线），不是 crossup / crossdown')
    w('enterShort = cross_up(close, VAH_)     // 上穿 VAH → 进入做空区')
    w('enterLong  = cross_down(close, VAL_)   // 下穿 VAL → 进入做多区')
    w('exitShort  = cross_down(close, VAH_)   // 回落价值区 → 平空')
    w('exitLong   = cross_up(close, VAL_)     // 回升价值区 → 平多')
    w()
    w('// ---------- 预警 ----------')
