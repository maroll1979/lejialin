...
`test-liq-*.js` 会真实联网；若本机只走代理，用 `NODE_USE_ENV_PROXY=1 node tests/xxx.js`。
网络不通时这几个脚本会 `fetch failed`（不是代码问题），可先 `curl https://api.gateio.ws/api/v4/futures/usdt/time` 确认。
